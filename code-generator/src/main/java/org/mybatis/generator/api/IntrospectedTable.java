...
public abstract class IntrospectedTable {
	...
	protected void calculateXmlAttributes() {
		this.setIbatis2SqlMapPackage(this.calculateIbatis2SqlMapPackage());
		...
		this.setMyBatis3XmlMapperPackage(this.getIbatis2SqlMapPackage()); // TODO? works
		// for now
		...
		// 真删除
		this.setDeleteByPrimaryKeyStatementId("deleteByIdReal"); //$NON-NLS-1$
		// 逻辑删除
		this.setDeleteByPrimaryKeyLogicStatementId("deleteById"); //$NON-NLS-1$
		// 全部字段依次插入
		this.setInsertStatementId("insertAllColumns"); //$NON-NLS-1$
		// 选择性插入
		this.setInsertSelectiveStatementId("insert"); //$NON-NLS-1$
		...
		this.setSelectByPrimaryKeyStatementId("selectById"); //$NON-NLS-1$
		...
		// 全部字段依次修改
		this.setUpdateByPrimaryKeyStatementId("updateByIdAllColumns"); //$NON-NLS-1$
		// 选择性修改
		this.setUpdateByPrimaryKeySelectiveStatementId("updateById"); //$NON-NLS-1$
		this.setUpdateByPrimaryKeyWithBLOBsStatementId("updateByPrimaryKeyWithBLOBs"); //$NON-NLS-1$
		this.setBaseResultMapId("BaseResultMap"); //$NON-NLS-1$
	...
	public void setDeleteByPrimaryKeyLogicStatementId(String s) {
		this.internalAttributes.put(InternalAttribute.ATTR_DELETE_BY_PRIMARY_KEY_LOGIC_STATEMENT_ID, s);
	}
	...
	public String getDeleteByPrimaryKeyLogicStatementId() {
		return this.internalAttributes.get(InternalAttribute.ATTR_DELETE_BY_PRIMARY_KEY_LOGIC_STATEMENT_ID);
	}
	...
	public boolean isConstructorBased() {
		...
		if (this.tableConfiguration.getProperties().containsKey(PropertyRegistry.ANY_CONSTRUCTOR_BASED)) {
			properties = this.tableConfiguration.getProperties();
		} else {
			properties = this.context.getJavaModelGeneratorConfiguration().getProperties();
		}
